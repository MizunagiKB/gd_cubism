...
void GDCubismUserModel::_update(const float delta) {
    ...
    // update parameters
    {
        Array ary_parameters = this->get_parameters().values();
        for (int i = 0; i < ary_parameters.size(); i++) {
            Dictionary param = ary_parameters[i];
    ...
    // update opacities
    {
        Array ary_parts = this->get_parts().values();
        for (int i = 0; i < ary_parts.size(); i++) {
            Dictionary part = ary_parts[i];
