...
public partial class viewer : Control
{
    ...
    public void setup(String pathname)
    {
        ...
        Godot.Collections.Dictionary canvas_info = this.cubism_model.GetCanvasInfo();
        GD.Print(canvas_info);

        int idx = 0;
        Godot.Collections.Dictionary<String, int> dict_motion = this.cubism_model.GetMotions();
        GD.Print(dict_motion);
        ...
        mat.BlendMode = CanvasItemMaterial.BlendModeEnum.PremultAlpha;
        mat.LightMode = CanvasItemMaterial.LightModeEnum.Unshaded;
        GetNode<Sprite2D>("Sprite2D").Material = mat;
    }
    
    public void model3_search(String dirname)
    {
        ...
        while(name != "")
        {
            if(dir.CurrentIsDir())
            {
                model3_search(dirname.PathJoin(name));
            } else {
    ...
    {
        Vector2 vct_resolution = new Vector2(GetWindow().Size.X, GetWindow().Size.Y);
        ...
        this.cubism_model.Size = new Vector2I((int)texture_height, (int)texture_height);

        Vector2 vct_viewport_size = new Vector2(GetViewportRect().Size.X, GetViewportRect().Size.Y);
